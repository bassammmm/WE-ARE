...
public class NexVersionHelper 
{
    ...
    public static void ChangeVersion(uint version)
    {
        ...
        {
            if (!IsAlreadySameVerion(applicationPath + folder, version))
            {
                string auxTempPath = applicationPath + tempPath + i; // create a new temporal path for each directory we want to change
                ...
                CopyDirectoryContent(applicationPath + folder, auxTempPath);

                // we delete the previous folder
                DeleteDirectory(applicationPath + folder); // TODO: in 2018, if you access the prefabs or scenes folder after importing the package, 
                // Unity editor will crash 

                // and create an empty one
                CreateDirectory(applicationPath + folder);

                // in order to move the version directory into it
                MoveDirectory(auxTempPath + versionPath, applicationPath + folder + '/' + version + '_');
                ...
                CreateAuxiliaryVersionFile(applicationPath + folder, version);
