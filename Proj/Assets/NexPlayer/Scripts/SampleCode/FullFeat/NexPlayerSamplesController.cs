...
namespace NexPlayerSample
{
    public class NexPlayerSamplesController : MonoBehaviour
    {
        ...
        public void AssignMaterials()
        {
            if (materials == null)
            {
                materials = (NexMaterials)AssetDatabase.LoadAssetAtPath("Assets/NexPlayer/Resources/NexMaterials.asset", typeof(NexMaterials));
            }
            materials.ResetReferences();
            if (sprites == null)
            {
                sprites = (NexUISprites)AssetDatabase.LoadAssetAtPath("Assets/NexPlayer/Resources/NexUISprites.asset", typeof(NexUISprites));
            }
            ...
            cube.GetComponent<MeshRenderer>().sharedMaterial = materials.Default;
            ...
            {
                multiplerenderers.GetChild(i).GetComponent<MeshRenderer>().sharedMaterial = materials.Default;
                multiplerenderers.GetChild(i).GetComponent<MeshFilter>().sharedMesh = materials.cube;
            }
            // Change RenderMode Sample
            transform.GetChild(6).GetChild(0).GetComponent<MeshRenderer>().sharedMaterial = materials.RenderTexture0;
            transform.GetChild(6).GetChild(0).GetComponent<MeshFilter>().sharedMesh = materials.screen;
            //transform.GetChild(6).GetChild(0).position += new Vector3(0, 0, 650);
            transform.GetChild(6).GetChild(1).GetComponent<MeshRenderer>().sharedMaterial = materials.Default;
            transform.GetChild(6).GetChild(1).GetComponent<MeshFilter>().sharedMesh = materials.cube;
            // 360 Sample
            ...
            np.cursorHandGrabbing = sprites.handGrab;
            np.cursorHandHovering = sprites.handHover;
            np.toggleUI = vrUi;
        ...
        public void SetNexPlayerForSample(NEXPLAYER_SAMPLES index)
        {
            ...
            if ((int)index < 7)
            {
                ...
                nexPlayer.URL = nexPlayer.streamingUri = "https://bitdash-a.akamaihd.net/content/MI201109210084_1/m3u8s/f08e80da-bf1d-4e3d-8899-f0f6155f6efa.m3u8";
            }
            else if ((int)index == 7)
            {
                SetMultistream(false);
                nexPlayer.URL = nexPlayer.streamingUri = "https://s3.eu-west-3.amazonaws.com/content.nexplayersdk.com/360test/NYCTimewarp/HLS-360/master.m3u8";
            }
            else
            {
                SetMultistream(true);
